...
flags.DEFINE_boolean('multiple_ckpts', False, 'Signify whether running inference on multiple ckpt models.')
flags.DEFINE_string('ckpt_dir', None, 'Path to checkpoint directory, only needed if running multiple ckpt models.')
flags.DEFINE_string('data_dir', None, 'Path to training dataset, only needed if running multiple ckpt models.')
...
def run_inference_experiment():
    ...
    ckpts = glob.glob(FLAGS.ckpt_dir + "/model-*")
    ...
    dataset_size = len(glob.glob(FLAGS.data_dir + "/*"))
    ...
    for i in range(len(EPOCHS)):
        if EPOCHS[i] < total_epochs:
            output_dir = os.path.join(FLAGS.output_dir, str(EPOCHS[i]) + "_epochs")

            # Find ckpt dir nearest to desired epoch
            model_ckpt = os.path.join(FLAGS.ckpt_dir,
                                      "model-{}".format(find_nearest(training_steps, EPOCHS[i] * dataset_size)))
