...
def display_images(image_ds):
    ...
    for image in enumerate(image_ds.take(1)):
        for n in range(BATCH_SIZE):
            fig.add_subplot(2, 3, n + 1)
            plt.imshow(tf.squeeze(image[1][n, :, :, 2]))
            plt.grid(False)
            plt.xticks([])
            plt.yticks([])
        plt.show()
...
def augment_image_colorspace(image_stack):
    ...
    image_stack_aug = tf.cond(
        apply_brightness,
        lambda: tf.image.random_brightness(image_stack, max_delta=0.1),
        lambda: image_stack)
...
class Cropper:
    ...
    def scale_and_crop_intrinsics(self, intrinsics):
        intrinsics = tf.map_fn(lambda x: self.scale_intrinsics_randomly(x), intrinsics)
        ...
        intrinsics = tf.map_fn(lambda x: make_intrinsics_matrix(x[0, 0], x[1, 1],
                                                                x[0, 2] - tf.cast(offset_x, dtype=tf.float32),
                                                                x[1, 2] - tf.cast(offset_y, dtype=tf.float32)), intrinsics)
        return intrinsics
...
data_root_seg = '/mnt/isaac/apps/carter_sim_struct2depth/sim_images/sim_images_25_delay'
...
seg_ds = seg_ds.batch(BATCH_SIZE, drop_remainder=True)
intrinsics_ds = intrinsics_ds.batch(BATCH_SIZE, drop_remainder=True)
...
with tf.name_scope('preprocessing'):
    ...
    intrinsics_ds = intrinsics_ds.map(lambda x: tf.reshape(x, [5, 3, 3]))
    ...
    # print("Unpacked images: {}".format(tf.data.get_output_shapes(image_stack_ds)))
    ...
    if FLIP:
        ...
        with tf.name_scope('image_augmentation_flip'):
            ...
            flipper = Flipper(image_width=WIDTH, batch_size=BATCH_SIZE, randomized=random_flipping)
    ...
    with tf.name_scope('shuffling'):
        if SHUFFLE:
            image_stack_ds = image_stack_ds.shuffle(buffer_size=SHUFFLE_BUFFER_SIZE, seed=2)
            image_stack_norm = image_stack_norm.shuffle(buffer_size=SHUFFLE_BUFFER_SIZE, seed=2)
            seg_stack_ds = seg_stack_ds.shuffle(buffer_size=SHUFFLE_BUFFER_SIZE, seed=2)
            intrinsics_ds = intrinsics_ds.shuffle(buffer_size=SHUFFLE_BUFFER_SIZE, seed=2)
            intrinsics_inv = intrinsics_inv.shuffle(buffer_size=SHUFFLE_BUFFER_SIZE, seed=2)
